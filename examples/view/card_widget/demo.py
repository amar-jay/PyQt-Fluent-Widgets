...
from PySide6.QtCore import QPoint, QPropertyAnimation, QRect, QSize, Qt, QUrl
...
class SystemRequirementCard(HeaderCardWidget):
	...
	def __init__(self, parent=None):
		...
		self.infoLabel = BodyLabel(
			"此产品适用于你的设备。具有复选标记的项目符合开发人员的系统要求。", self
		)
...
class SettinsCard(GroupHeaderCardWidget):
	def __init__(self, parent=None):
		...
		self.addGroup(
			"resource/Joystick.svg", "运行终端", "设置是否显示命令行终端", self.comboBox
		)
		self.addGroup(
			"resource/Python.svg", "入口脚本", "选择软件的入口脚本", self.lineEdit
		)
...
class Demo2(MicaWindow):
	def __init__(self):
		...
		self.addCard(
			f":/qfluentwidgets/images/logo.png",
			"PyQt-Fluent-Widgets",
			"Shokokawaii Inc.",
		)
		self.addCard(
			f"{suffix}/TitleBar.png", "PyQt-Frameless-Window", "Shokokawaii Inc."
		)
		self.addCard(f"{suffix}/RatingControl.png", "反馈中心", "Microsoft Corporation")
		self.addCard(
			f"{suffix}/Checkbox.png", "Microsoft 使用技巧", "Microsoft Corporation"
		)
		self.addCard(f"{suffix}/Pivot.png", "MSN 天气", "Microsoft Corporation")
		self.addCard(
			f"{suffix}/MediaPlayerElement.png", "电影和电视", "Microsoft Corporation"
		)
...
class Demo3(MSFluentWindow):
	def __init__(self, parent=None):
		...
		self.navigationInterface.addItem(
			"editInterface", FluentIcon.EDIT, "编辑", selectable=False
		)
