...
class Demo1(QWidget):
	def __init__(self):
		...
		self.commandBar.addAction(
			Action(FluentIcon.EDIT, "Edit", triggered=self.onEdit, checkable=True)
		)
		...
		self.commandBar.addHiddenAction(
			Action(FluentIcon.SCROLL, "Sort", triggered=lambda: print("排序"))
		)
		self.commandBar.addHiddenAction(
			Action(FluentIcon.SETTING, "Settings", shortcut="Ctrl+S")
		)
